# `@bridgebyte/sst-dom` Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a vanilla-DOM table variant `@bridgebyte/sst-dom` that renders an `ITableStore<T>` into a container element with a single `mountTable({ target, store, columns })` call — header, sortable columns, body rows, and pagination controls — keeping `@bridgebyte/sst-core` framework-agnostic.

**Architecture:** New workspace package `packages/dom/` consuming `@bridgebyte/sst-core` as a peer dependency. Pure TypeScript, browser-only, ships its own `style.css`. Tested with `happy-dom`. Rendering split into focused units (`render-head`, `render-body`, `render-pagination`) that `mount-table.ts` glues together with subscriptions to the store's observables. Includes a small breaking change in core (drop unused `IColumn.render: boolean`) and a bump from 0.1.0 → 0.2.0.
...
- `@bridgebyte/sst-core@0.1.0` is already published to local Verdaccio.
- Node 20+, npm 10+.
...
```
packages/dom/
├── package.json                       # name "@bridgebyte/sst-dom", peerDeps @bridgebyte/sst-core
├── tsconfig.json
├── tsconfig.build.json
...
- [ ] **Step 2: Bump `@bridgebyte/sst-core` version**
...
Expected: `+ @bridgebyte/sst-core@0.2.0`. If Verdaccio rejects with "already exists", run `npm run registry` first to ensure it's up.
...
## Task 2: Scaffold `@bridgebyte/sst-dom` package
...
```json
{
  "name": "@bridgebyte/sst-dom",
  "version": "0.1.0",
  "description": "So Simple Table — vanilla DOM variant. Renders an @bridgebyte/sst-core TableStore as an HTML table.",
  "type": "module",
  "main": "./dist/index.cjs",
...
```ts
// @bridgebyte/sst-dom public API — populated by subsequent tasks.
export {};
```
...
```css
/* @bridgebyte/sst-dom — populated by Task 9. */
```
...
Vanilla DOM variant of [@bridgebyte/sst-core](https://www.npmjs.com/package/@bridgebyte/sst-core).
See `01-core.md` and the `@bridgebyte/sst-dom` design spec for context.
...
Add the `@bridgebyte/sst-dom` path mapping. Edit `tsconfig.base.json`:

```json
"paths": {
  "@bridgebyte/sst-core": ["packages/core/src/index.ts"],
  "@bridgebyte/sst-dom": ["packages/dom/src/index.ts"]
}
```
...
git commit -m "feat(dom): scaffold @bridgebyte/sst-dom package"
```
...
Expected: `+ @bridgebyte/sst-dom@0.1.0`.

- [ ] **Step 6: Verify it's in the registry**

```bash
curl -s http://localhost:4873/@bridgebyte%2Fsst-dom | grep -E '"name"|"version"|"latest"' | head -5
```

Expected output includes `"name": "@bridgebyte/sst-dom"` and `"latest": "0.1.0"`.
...
The previous app manually rendered a `<table>` and pagination. With `@bridgebyte/sst-dom`, all that becomes one `mountTable(...)` call. We keep the search and status-filter inputs (those exercise core's reactivity outside `@bridgebyte/sst-dom`'s scope).
...
```json
{
  "name": "@bridgebyte/sst-test-dom",
  "private": true,
  "version": "0.0.0",
  "type": "module",
  "description": "Browser smoke-test app for @bridgebyte/sst-core + @bridgebyte/sst-dom consumed via the local Verdaccio registry.",
  "scripts": {
    "dev": "vite",
...
The inline `<table>`, pagination div, totals/page indicator, and the sort `<select>` are gone — `@bridgebyte/sst-dom` produces them. Keep search + status filter inputs and the call log.
...
<html lang="en">
	<head>
		...
		<title>@bridgebyte/sst-dom — browser smoke test</title>
		<link rel="stylesheet" href="./src/style.css" />
	</head>
	<body>
		<main>
			<header>
				<h1>@bridgebyte/sst-dom — browser smoke test</h1>
				<p id="lib-version">loading library…</p>
			</header>

			<section id="exports">
				<h2>1. Imported exports (@bridgebyte/sst-core)</h2>
				<ul id="exports-list"></ul>
			</section>

			<section id="store">
				<h2>2. mountTable(@bridgebyte/sst-dom) over a stub repository</h2>
				<div class="controls">
...
// 3. Wire search + filter inputs to drive the store directly (NOT @bridgebyte/sst-dom's job).
...
- **Import smoke** — lists every export pulled from `@bridgebyte/sst-core`.
...
- **App-side wiring** — search input and status filter `<select>` drive
  ...
  intentionally NOT part of `@bridgebyte/sst-dom`.
...
Expected: `npm install` resolves `@bridgebyte/sst-dom@0.1.0` from Verdaccio. `npm run build` succeeds (TypeScript clean, Vite produces `dist/`).
...
git status
git commit -m "test: replace test/core with test/dom — uses @bridgebyte/sst-dom mountTable"
```

---

## Task 12: Write the real `@bridgebyte/sst-dom` README
...
git commit -m "docs(dom): add @bridgebyte/sst-dom README with usage and API reference"
```
...
1. **Spec coverage**
	- ✅ Sibling package `@bridgebyte/sst-dom`, peer-dep on core — Tasks 2, 10.
...
**Plan complete.** Continue to `02-angular.md` after this lands (Angular variant inherits the same architecture and can reference `@bridgebyte/sst-dom`'s tests as a precedent for sort-cycle semantics).
